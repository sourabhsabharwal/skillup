view: self_assigned_cert_pre_req_skill_status_data {
      derived_table: {
      sql: with self_assigned_cert_data as
              (
              select
              distinct e.id as emp_id,
              e.email,
              ...
              where
              cert_company not in (371416, 364822, 327186, 297361, 280588, 280251, 279841, 178821, 163391, 118702, 0, 279841)
              ) cert_pre_skill_mapping
              ...
              select
                  ...
                  employee_id,
                  target_prof,
                  case when max(passed) = 1 then true else false end as prof_achieved
                  from self_assigned_cert_data
